...
def getArticles(genre, orderBy):
    ...
    sortList = []
    for a in cursor:
        sortList.append([a['ArticleID'], "<div ><a href='/Articles/%s'><img class='article_img' src='%s'><br>%s</a></div>" % (str(a['ArticleID']), str(a['img_url']),str(a['title']))])
...
def sortBy(currentSort):
    ...
    return '''<label>Sort By</label>
                ...
                <!--Creates the list of things to display in the drop down box-->
                    <option value = "0"%s>Newest to Oldest</option>
                    <option value = "1"%s>Oldest to Newest</option>
